...
///
/// Lookups go from most to least precise, stopping at the first hit:
...
/// Every request is a single attempt with a short timeout — no retry/backoff
/// — and the whole lookup is capped at `overallBudget` regardless of how many
/// stages it falls through, so a slow or flaky lrclib response can only ever
/// delay the overlay briefly, never hang it.
final class LyricsService {
    ...
    private static let overallBudget: TimeInterval = 6
    ...
    func fetchLyrics(
        ...
        Task {
            let lines = await withTaskGroup(of: [LyricLine].self) { group in
                group.addTask { await self.lookup(track: track, artist: artist, album: album, duration: duration) }
                group.addTask {
                    try? await Task.sleep(nanoseconds: UInt64(Self.overallBudget * 1_000_000_000))
                    return []
                }
    ...
    /// GET + decode, a single attempt (no retry — the caller's overall
    /// budget bounds total time, and a miss here just falls through to the
    /// next, differently-shaped query instead).
